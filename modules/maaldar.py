...
from modules.palette import DropdownViewPalette
from modules.palette import Palette
from util import DropdownView
from util import check_if_user_exists
from util import clean_up
from util import concatenate_images
...
class Maaldar(commands.Cog):
    ...
    @maaldar_group.command(name="role", description="Creates a new role for you")
    @app_commands.describe(name="Name of the role you want to create")
    ...
    async def role(self, interaction: discord.Interaction, name: str) -> None:
        ...
        if maaldar_user is None:
            try:
                ...
                await role.edit(
                    position=(
                        guild.get_role(configuration["custom_role_id"]).position - 1
                    )
                )
                ...
                await interaction.followup.send(
                    f"**{name}** created and assigned to you ✨"
                )
                return
    ...
    @maaldar_group.command(name="name", description="Sets a new name for your role")
    @app_commands.describe(new_name="Name of the role")
    @app_commands.checks.has_any_role(*configuration["role_ids"])
    ...
    @app_commands.describe(color="New color for your role (e.g #000000)")
    @app_commands.checks.has_any_role(*configuration["role_ids"])
    ...
    @app_commands.describe(url="URL link to the icon (must be in PNG/JPG format)")
    @app_commands.checks.has_any_role(*configuration["role_ids"])
    ...
    @maaldar_group.command(name="assign", description="Assigns your role to you")
    @app_commands.describe(user="User to assign the role to")
    @app_commands.checks.has_any_role(*configuration["role_ids"])
    async def assign(
    ...
    @maaldar_group.command(name="unassign", description="Unassigns your role from you")
    @app_commands.describe(user="User to unassign the role from")
    @app_commands.checks.has_any_role(*configuration["role_ids"])
    async def unassign(
        self, interaction: discord.Interaction, user: discord.Member = None
    ) -> None:
    ...
    @maaldar_group.command(
        name="palette", description="Gets a color palette for your profile picture"
    )
    @app_commands.checks.has_any_role(*configuration["role_ids"])
    ...
    """
        if self.palette.usage > 5 or user_reattempt:
            ...
            await interaction.followup.send(file=discord.File("./palette.png"))
            clean_up()
        ...
        for image in images:
            ...
            emojis.append(image_byte_array.getvalue())
    ...
    async def commands_error(
        ...
        if isinstance(error, discord.app_commands.errors.MissingAnyRole):
            await interaction.response.send_message(
                "You need to be boosting the server to use this command", ephemeral=True
            )
            return
...
async def setup(bot: commands.Bot):
    await bot.add_cog(
        Maaldar(bot), guilds=[discord.Object(id=configuration["guild_id"])]
    )

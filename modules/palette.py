...
class Dropdown(discord.ui.Select):
    def __init__(self, *args):
        if len(args) == 4:
            ...
            options = [
                discord.SelectOption(
                    label=f"{args[0][index]}",
                    description=f"Color #{index + 1}",
                    emoji=emoji,
                )
        ...
        super().__init__(
            placeholder="Colors", min_values=1, max_values=1, options=options
        )
        ...
        try:
            if interaction.user == self.user:
                await self.role.edit(
                    color=discord.Color(int(self.values[0].strip("#"), 16))
                )

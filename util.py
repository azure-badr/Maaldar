...
def check_if_user_exists(cursor: Cursor, user_id):
    cursor.execute(f"SELECT * FROM Maaldar WHERE user_id = {user_id}")
    return cursor.fetchone()


def make_image(dominant_color):
    image = Image.new(mode="RGBA", size=(50, 50), color=(0, 0, 0, 0))
    ImageDraw.Draw(image).rounded_rectangle(
        (0, 0, 50, 50), radius=20, fill=dominant_color
    )
...
def concatenate_images(images):
    ...
    image = Image.new(mode="RGBA", size=(50 * 10, 50), color=(0, 0, 0, 0))
    ImageDraw.Draw(image).rounded_rectangle((0, 0, 500, 50), radius=20)
